...
export default function () {
    ...
    return <div>
        {!session?.data?.user ? <Loader /> :
            <form onSubmit={handleSubmit} className="max-w-7xl mx-auto py-9 bg-white rounded-lg shadow-2xl mt-20">
                ...
                <div className="flex justify-center mt-9">
                    <button className="rounded-md font-bold bg-green-700 p-2 w-48 text-white" type="submit" >{loading ? <Loader /> : 'Create'}</button>
                </div>
            </form>
        }
    </div>
